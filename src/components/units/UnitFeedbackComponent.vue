<template>
    <div class="main__container" style="margin: -2rem auto" v-if="exist">
        <div class="feedback__form">
            <div class="tablet_mobile__head">
                ...
                <div class="staff__card" v-if="unit !== null">
                    <div>
                        <div class="establishment__info">
                            <h1 class="society__name">{{ unit.establishment_name }}</h1>
                        </div>
    ...
    <ModalComponent :showModal="showModal" @close="showModal = false" :width="modalWidth">
        <template #content>
            ...
            <div class="modal__container" v-if="units.length > 0">
                <RouterLink v-for="unit_item in units" :key="unit_item.id" class="staff__card mb-1" :to="{
                    name: 'UnitFeedBack',
                    params: {
                        tag: route.params.tag,
                        etab: unit_item.establishment_competitor_tag,
                        id: unit_item.tag
                    }
                }" target="_blank">
...
import { ref, onBeforeMount, defineAsyncComponent, computed, onMounted, inject, watch } from 'vue';
                        ...
                        await services.createRecord('public/contacts', contactData, async (contactResponse) => {
                            if (contactResponse.status == 201) {
                                if (visitorId) {
                                    ...
                                    services.post_Record('public/visitors', visitor, (res) => {
                                        console.log(res)
                                    }, true)
                                }
