...
export const useCompanyStore = defineStore("company", {
  ...
  actions: {
    ...
    getTopThreeReviews(reviews, rating, result) {
      reviews.forEach((review) => {
        if (Number(review.rating) > 5) {
          if (Number(review.rating) / 2 == rating && result.length < 3)
            result.push(review);
        } else {
          if (Number(review.rating) == rating && result.length < 3)
            result.push(review);
        }
      });
      rating--;
      if (result.length < 3 && rating >= 0)
        this.getTopThreeReviews(reviews, rating, result);
      return result;
    },
    ...
    splitRangeIntoSemesters(start_date, end_date) {
      ...
      const numberOfSemesters = Math.ceil(
        totalDuration / ((365 * 24 * 60 * 60 * 1000) / 2)
      );
      ...
      for (let i = 0; i < numberOfSemesters; i++) {
        const semesterStart = new Date(
          startDate.getTime() + i * semesterDuration
        );
        const semesterEnd = new Date(
          semesterStart.getTime() + semesterDuration
        );
